...
def getNodesWithChildren(inpt):
  nodesWithKids = []

  for item in inpt:
    cur = item.split('->')

    if len(cur) > 1: # it has children
      temp = Tree()
      temp.name = cur[0].split(' (')[0]
      
      for child in cur[1].split(','):
        temp.children.append(child[1:])

      nodesWithKids.append(temp)

  return nodesWithKids

def returnBase(inpt):
  nodes = getNodesWithChildren(inpt)
  retVal = ''
  i = 0
  foundBase = False

  while not foundBase and i < len(nodes):
    curName = nodes[i].name

    inChildren = False
    j = 0

    while not inChildren and j < len(nodes):
      if j == i:
        j += 1
      inChildren = determineIfInChildren(curName, nodes[j].children)
      j += 1
...
def determineIfInChildren(name, children):
  found = False
  i = 0
...
if __name__ == "__main__":
  print(returnBase(test))
  print(returnBase(actual))
